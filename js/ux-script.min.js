...
LazyLoad=function(doc){function createNode(name,attrs){var attr,node=doc.createElement(name);for(attr in attrs)attrs.hasOwnProperty(attr)&&node.setAttribute(attr,attrs[attr]);return node}function finish(type){var callback,urls,p=pending[type];p&&(callback=p.callback,urls=p.urls,urls.shift(),pollCount=0,urls.length||(callback&&callback.call(p.context,p.obj),pending[type]=null,queue[type].length&&load(type)))}function getEnv(){var ua=navigator.userAgent;env={async:doc.createElement("script").async===!0},(env.webkit=/AppleWebKit\//.test(ua))||(env.ie=/MSIE|Trident/.test(ua))||(env.opera=/Opera/.test(ua))||(env.gecko=/Gecko\//.test(ua))||(env.unknown=!0)}function load(type,urls,callback,obj,context){var i,len,node,p,pendingUrls,url,_finish=function(){finish(type)},isCSS="css"===type,nodes=[];if(env||getEnv(),urls)if(urls="string"==typeof urls?[urls]:urls.concat(),isCSS||env.async||env.gecko||env.opera)queue[type].push({urls:urls,callback:callback,obj:obj,context:context});else for(i=0,len=urls.length;len>i;++i)queue[type].push({urls:[urls[i]],callback:i===len-1?callback:null,obj:obj,context:context});if(!pending[type]&&(p=pending[type]=queue[type].shift())){for(head||(head=doc.head||doc.getElementsByTagName("head")[0]),pendingUrls=p.urls.concat(),i=0,len=pendingUrls.length;len>i;++i)url=pendingUrls[i],isCSS?node=env.gecko?createNode("style"):createNode("link",{href:url,rel:"stylesheet"}):(node=createNode("script",{src:url}),node.async=!1),node.className="lazyload",node.setAttribute("charset","utf-8"),env.ie&&!isCSS&&"onreadystatechange"in node&&!("draggable"in node)?node.onreadystatechange=function(){/loaded|complete/.test(node.readyState)&&(node.onreadystatechange=null,_finish())}:isCSS&&(env.gecko||env.webkit)?env.webkit?(p.urls[i]=node.href,pollWebKit()):(node.innerHTML='@import "'+url+'";',pollGecko(node)):node.onload=node.onerror=_finish,nodes.push(node);for(i=0,len=nodes.length;len>i;++i)head.appendChild(nodes[i])}}function pollGecko(node){var hasRules;try{hasRules=!!node.sheet.cssRules}catch(ex){return pollCount+=1,void(200>pollCount?setTimeout(function(){pollGecko(node)},50):hasRules&&finish("css"))}finish("css")}function pollWebKit(){var i,css=pending.css;if(css){for(i=styleSheets.length;--i>=0;)if(styleSheets[i].href===css.urls[0]){finish("css");break}pollCount+=1,css&&(200>pollCount?setTimeout(pollWebKit,50):finish("css"))}}var env,head,pending={},pollCount=0,queue={css:[],js:[]},styleSheets=doc.styleSheets;return{css:function(urls,callback,obj,context){load("css",urls,callback,obj,context)},js:function(urls,callback,obj,context){load("js",urls,callback,obj,context)}}}(this.document);
var BrowserDetect={init:function(){this.browser=this.searchString(this.dataBrowser)||"An unknown browser",this.version=this.searchVersion(navigator.userAgent)||this.searchVersion(navigator.appVersion)||"an unknown version",this.OS=this.searchString(this.dataOS)||"an unknown OS"},searchString:function(data){for(var i=0;i<data.length;i++){var dataString=data[i].string,dataProp=data[i].prop;if(this.versionSearchString=data[i].versionSearch||data[i].identity,dataString){if(-1!=dataString.indexOf(data[i].subString))return data[i].identity}else if(dataProp)return data[i].identity}},searchVersion:function(dataString){var index=dataString.indexOf(this.versionSearchString);if(-1!=index)return parseFloat(dataString.substring(index+this.versionSearchString.length+1))},dataBrowser:[{string:navigator.userAgent,subString:"Chrome",identity:"Chrome"},{string:navigator.userAgent,subString:"OmniWeb",versionSearch:"OmniWeb/",identity:"OmniWeb"},{string:navigator.vendor,subString:"Apple",identity:"Safari",versionSearch:"Version"},{prop:window.opera,identity:"Opera",versionSearch:"Version"},{string:navigator.vendor,subString:"iCab",identity:"iCab"},{string:navigator.vendor,subString:"KDE",identity:"Konqueror"},{string:navigator.userAgent,subString:"Firefox",identity:"Firefox"},{string:navigator.vendor,subString:"Camino",identity:"Camino"},{string:navigator.userAgent,subString:"MSIE",identity:"MSIE",versionSearch:"MSIE"},{string:navigator.userAgent,subString:"Gecko",identity:"Mozilla",versionSearch:"rv"}],dataOS:[{string:navigator.platform,subString:"Win",identity:"Windows"},{string:navigator.platform,subString:"Mac",identity:"Mac"},{string:navigator.userAgent,subString:"iPhone",identity:"iPhone/iPod"},{string:navigator.platform,subString:"Linux",identity:"Linux"}]};BrowserDetect.init();
function switch_selected(newSelection){jQuery(".ux-a-z-index a.selected").removeClass("selected"),jQuery(".ux-a-z-item-container.selected").fadeOut("fast",function(){jQuery(this).removeClass("selected"),jQuery("#"+newSelection+"-index").addClass("selected"),jQuery("#"+newSelection+"-container").fadeIn("fast",function(){jQuery(this).addClass("selected")})})}function inputDisabledStyle(){jQuery("input:disabled").addClass("disabled"),jQuery("input:enabled").removeClass("disabled")}if(!window.ux){window.ux={},window.ux.core={},window.ux.features={},window.ux.libraries={},window.ux.functions={},window.ux.dom={},ux.core.browser=BrowserDetect.browser.toLowerCase(),ux.core.protocol=location.protocol,ux.core.extPrefix=".min",ux.core.cssFolderSuffix="",ux.core.jsFolderSuffix="",ux.core.devMode=!1;var scriptEls=document.getElementsByTagName("script"),thisScriptEl=scriptEls[scriptEls.length-1],scriptPath=thisScriptEl.src;ux.core.resourcePath=scriptPath.substr(0,scriptPath.lastIndexOf("/v.")+9),ux.core.version=ux.core.resourcePath.substr(scriptPath.lastIndexOf("/v.")+1,scriptPath.lastIndexOf("/v.")+9).replace("/",""),ux.features.alias=function(alias,loc){window.ux[alias]?console.log("Sorry you cant make a shortcute to:"+alias):window.ux[alias]=loc},ux.features.domSaver=function(searchItem,searchName){ux.dom[searchName]=jQuery(searchItem)},ux.features.startup=function(callback){void 0!=window.beforeUX&&"function"==typeof window.beforeUX&&beforeUX(),jQuery("body").addClass(ux.core.browser),jQuery.extend({getUrlVars:function(){for(var hash,vars=[],hashes=window.location.href.slice(window.location.href.indexOf("?")+1).split("&"),i=0;i<hashes.length;i++)hash=hashes[i].split("="),vars.push(hash[0]),vars[hash[0]]=hash[1];return vars},getUrlVar:function(name){return jQuery.getUrlVars()[name]}}),("chrome"==ux.core.browser||"safari"==ux.core.browser||"opera"==ux.core.browser)&&(jQuery("#skip-to-content").on("click",function(){document.getElementById("content-anchor").focus()}),jQuery("#skip-to-menu").on("click",function(){document.getElementById("domain-menu-anchor").focus()}),jQuery("#skip-to-nav").on("click",function(){document.getElementById("navigation-anchor").focus()})),ux.features.alias("status",ux.features.status),ux.features.alias("load",ux.features.load),ux.features.alias("domSaver",ux.features.domSaver),ux.features.alias("override",ux.features.override),jQuery("#ux-side-nav ul li:first-child, #ux-footer ul li:first-child").addClass("first-child"),jQuery("#ux-side-nav ul li:last-child, #ux-footer ul li:last-child").addClass("last-child"),jQuery(".dol-container .dol-container:first-child").addClass("first-child"),jQuery(".dol-container .dol-container:last-child").addClass("last-child"),jQuery(".layout-table-content>.layout-2-col:first-child").addClass("first-child"),jQuery(".layout-table-content>.layout-2-col:last-child").addClass("last-child"),jQuery("table tr:last-child").addClass("last-child"),jQuery("thead th:last-child").addClass("last-child"),jQuery("tr td:last-child").addClass("last-child"),jQuery("table.row-stripe tr:odd").addClass("table-row-odd"),jQuery("table.row-stripe tr:even").addClass("table-row-even"),"msie"==ux.core.browser&&jQuery("body").hasClass("ie7")&&(jQuery(".ie7 tfoot tr:last-child").addClass("last-child"),jQuery(".ie7 .ux-wizard li:last-child").addClass("last-child")),"function"==typeof callback&&callback(),document.getElementById("ux-side-nav")&&ux.load("expandables"),document.getElementById("ux-side-nav-container")&&document.getElementById("ux-side-nav-toggle")&&jQuery("#ux-side-nav-toggle").on("click",function(e){e.preventDefault(),e.stopPropagation(),jQuery("#ux-domain-menu a.selected").length>0&&jQuery("#ux-domain-menu a.selected").removeClass("selected").attr("aria-expanded",!1).next("div:first").hide().attr("aria-hidden",!0);var link=jQuery(this),container=jQuery("#ux-side-nav-container");container.is(":visible")?(link.removeClass("active").attr("aria-expanded",!1),container.hide().attr("aria-hidden",!0),jQuery(document).off("click"),jQuery(document).off("keyup")):(link.addClass("active").attr("aria-expanded",!0),container.show().attr("aria-hidden",!1),jQuery(document).on("click",function(){link.removeClass("active").attr("aria-expanded",!1),container.hide().attr("aria-hidden",!0),jQuery(this).off("click")}),document.onkeyup=function(evt){evt=evt||window.event,27==evt.keyCode&&jQuery("#ux-side-nav-toggle").removeClass("active").attr("aria-expanded",!1).next("div:first").hide().attr("aria-hidden",!0).focus()})}),jQuery(".ux-a-z-index").length&&jQuery(".ux-a-z-index a").each(function(){jQuery(this).on("click",function(event){event.preventDefault();var selection=jQuery(this).text();"#"==selection&&(selection="numbers"),switch_selected(selection)})}),jQuery("#gov-link3").length&&jQuery("#gov-link3").on("click",function(e){e.preventDefault(),jQuery(this).hide().next("form").show().focus()}),ux.domSaver(".ux-content a, footer a","uxPageAnchors"),ux.load(["placeHolder","tooltips"]),jQuery("#app-nav").length&&ux.load("appMenu"),jQuery(".processing").length&&ux.load("modalLoader")},ux.features.status=function(){var status="Current state of this UX page:\n\nUX version: "+ux.core.version+"\nProtocal used: "+ux.core.protocol+"\nBrowser detected: "+ux.core.browser+"\nFile Extention Prefix: "+ux.core.extPrefix+"\nFolder Suffix: "+ux.core.folderSuffix+"\nAsset Resource Path: "+ux.core.resourcePath+"\n\nLazyLoadable Library State:\n\n";for(func in ux.libraries)status+=func+": "+ux.libraries[func].isLoaded+"\n";status+="\n\nFunction State:\n\n";for(func in ux.functions)status+=func+": "+ux.functions[func].isLoaded+"\n";return console.log(status),!0},ux.features.load=function(request,callback){function testRequest(request,callback){if(request in ux.libraries)if(ux.libraries[request].isLoaded)ux.libraries[request].hasOwnProperty("deprecated")&&console.log(ux.libraries[request].deprecated);else{if(ux.libraries[request].hasOwnProperty("dependencies")){typeof ux.libraries[request].dependencies;ux.features.load(ux.libraries[request].dependencies)}loader(!0,ux.libraries[request],callback)}else request in ux.functions?executer(request,callback):console.log("Unsupported items ("+request+") requested. Check the syntax for a supported library or directly load item using the lazy load script not the ux.load")}function loader(trusted,request,callback){if(trusted){if(request.hasOwnProperty("styles")){var styles=request.styles;if("string"==typeof styles)styleChecker(styles)&&LazyLoad.css(assetPath(styles,"css"));else for(var i=0,r=styles.length;r>i;i++)styleChecker(styles[1])&&LazyLoad.css(assetPath(styles[i],"css"))}if(request.hasOwnProperty("script")){var afterLazyLoad=function(){request.hasOwnProperty("defaultFunction")&&request.defaultFunction(),"function"==typeof callback&&callback(),request.isLoaded=!0};request.hasOwnProperty("specialPath")?LazyLoad.js(assetPath(request.script,"js",request.specialPath),afterLazyLoad):LazyLoad.js(assetPath(request.script,"js"),afterLazyLoad)}}}function executer(request,callback){ux.functions[request].isLoaded||(ux.functions[request].defaultFunction(),ux.functions[request].isLoaded=!0)}function assetPath(request,type,specialPath){var version="",folder="";if(ux.core.devMode)var version=Math.floor(1001*Math.random());return folder+="js"==type?type+ux.core.jsFolderSuffix:type+ux.core.cssFolderSuffix,specialPath?ux.core.resourcePath+"/"+folder+"/"+specialPath+"/"+request+ux.core.extPrefix+"."+type+"?version="+version:ux.core.resourcePath+"/"+folder+"/"+request+ux.core.extPrefix+"."+type+"?version="+version}function styleChecker(file){return jQuery("link[href*="+file+"]").length>=1?!1:!0}if("string"==typeof request)testRequest(request,callback);else if("object"==typeof request)if(request instanceof Array)for(var i=0,r=request.length;r>i;i++)"string"==typeof request[i]?testRequest(request[i]):"object"==typeof request[i]&&ux.features.load(request[i]);else if("load"in request){var cbFunction="callback"in request?request.callback:void 0;testRequest(request.load,cbFunction)}},ux.features.override=function(request,obj,act){function overrideAction(origObj,newObj,act){if(act){for(member in origObj)delete member;origObj=jQuery.extend(origObj,newObj)}else origObj=jQuery.extend(origObj,newObj)}request in ux.libraries&&"defaultSettings"in ux.libraries[request]?overrideAction(ux.libraries[request].defaultSettings,obj,act):request in ux.functions&&"defaultSettings"in ux.functions[request]&&overrideAction(ux.functions[request].defaultSettings,obj,act)},ux.libraries={dataTables:{script:"lazy-jquery.dataTables",styles:["lazy-jquery-dataTables"],dependencies:"jqueryUI",defaultElement:".jQDataTables",defaultSettings:{bPaginate:!0,sPaginationType:"full_numbers",bSort:!0,aaSorting:[],bInfo:!0,bAutoWidth:!0,bLengthChange:!1,bFilter:!0,bRetrieve:!0,bJQueryUI:!0},defaultFunction:function(){function jQDTExpand_FormatDetails(targetDiv){var expandData=targetDiv.html(),sOut='<div class="ux-jQDT-row-expanded-content">'+expandData+"</div>";return sOut}jQuery(".ux-jQDT-row-expandable").each(function(){var trigger=jQuery(this).find("a.ux-jQDT-row-expand-trigger");trigger.prepend('<img src="'+ux.core.resourcePath+'/images/icons/add.png" class="ux-jQDT-row-expand-img">'),jQuery(this).find(".ux-jQDT-row-expand-content").hide()});var jQDTExpand_icon_closed=ux.core.resourcePath+"/images/icons/add.png",jQDTExpand_icon_opened=ux.core.resourcePath+"/images/icons/delete.png";jQuery("a.ux-jQDT-row-expand-trigger").on("click",function(){var thisTr=jQuery(this).closest("tr"),nTr=thisTr[0],targetDiv=jQuery(this).next(".ux-jQDT-row-expand-content"),oTable=jQuery(this).closest("table.data-table").dataTable({bRetrieve:!0}),trig_img=jQuery(this).find("img.ux-jQDT-row-expand-img")[0];return trig_img.src.match(jQDTExpand_icon_opened)?(trig_img.src=jQDTExpand_icon_closed,oTable.fnClose(nTr)):(jQuery(thisTr).find("a.ux-jQDT-row-expand-trigger").each(function(){jQuery(this).find("img.ux-jQDT-row-expand-img")[0].src=jQDTExpand_icon_closed}),oTable.fnClose(nTr),trig_img.src=jQDTExpand_icon_opened,oTable.fnOpen(nTr,jQDTExpand_FormatDetails(targetDiv),"ux-jQDT-row-expanded")),!1}),jQuery.extend(jQuery.fn.dataTable.defaults,this.defaultSettings),jQuery(this.defaultElement).dataTable()},isLoaded:!1},jqueryUI:{script:"lazy-jquery-ui-core",styles:["lazy-jquery-ui"],isLoaded:!1},jqueryTabs:{script:"lazy-jquery-ui-tabs",defaultElement:".jQTabs",defaultSettings:{},defaultFunction:function(){jQuery(this.defaultElement).tabs(this.defaultSettings)},dependencies:"jqueryUI",isLoaded:!1},jqueryDatePicker:{script:"lazy-jquery-ui-datepicker",defaultElement:".jQDatePicker",defaultSettings:{showAnim:"",showOn:"both",buttonText:"Datepicker",buttonImage:ux.core.resourcePath+"/images/icons/calendar_disabled.png",buttonImageOnly:!0,changeMonth:!0,changeYear:!0},defaultFunction:function(){jQuery(this.defaultElement).datepicker(this.defaultSettings)},dependencies:"jqueryUI",isLoaded:!1},jqueryAccordion:{script:"lazy-jquery-ui-accordion",dependencies:"jqueryUI",isLoaded:!1},jqueryAutoComplete:{script:"lazy-jquery-ui-autocomplete",dependencies:"jqueryUI",isLoaded:!1},jqueryButton:{script:"lazy-jquery-ui-button",dependencies:"jqueryUI",isLoaded:!1},jqueryDialog:{script:"lazy-jquery-ui-dialog",dependencies:"jqueryButton",isLoaded:!1},jquerySlider:{script:"lazy-jquery-ui-slider",dependencies:"jqueryUI",isLoaded:!1},jqueryProgressBar:{script:"lazy-jquery-ui-progressbar",dependencies:"jqueryUI",isLoaded:!1},jqueryDynaTree:{script:"lazy-jquery.dynatree",styles:"lazy-jquery-dynatree",dependencies:"jqueryUI",isLoaded:!1},formEnhancements:{deprecated:"This functionality is no longer avaliable please lookup see the documentation about the split of show hide password and character counter functionality",isLoaded:!0},characterCounter:{script:"lazy-jquery.open-ux.character-counter",styles:["lazy-jquery-open-ux-character-counter"],isLoaded:!1},showHidePassword:{script:"lazy-jquery.open-ux.show-hide-password",styles:"lazy-jquery-open-ux-show-hide-password",defaultElement:".ux-password-show-hide",defaultSettings:{moreClasses:"button_mini no-icon"},defaultFunction:function(){jQuery(this.defaultElement).showHidePass(this.defaultSettings)},isLoaded:!1},keyboardShortcuts:{script:"lazy-jquery.open-ux.keyboard-shortcuts",styles:"lazy-jquery-open-ux-keyboard-shortcuts",dependencies:"jqueryDialog",isLoaded:!1},notifications:{script:"lazy-jquery.open-ux.notifications",styles:"lazy-jquery-open-ux-notifications",isLoaded:!1},navOutWarning:{script:"lazy-jquery.open-ux.navigational-warning",styles:"lazy-jquery-open-ux-navigational-warning",dependencies:"jqueryDialog",defaultSettings:{},defaultFunction:function(selector){selector||(ux.domSaver("a","uxAllAnchors"),selector=ux.dom.uxAllAnchors),jQuery(selector).navOutWarning(this.defaultSettings)},isLoaded:!1},fullCalendar:{script:"lazy-jquery.fullcalendar",styles:["lazy-fullcalendar"],dependencies:"jqueryUI",isLoaded:!1},video:{script:"lazy-video",styles:"lazy-video-js",isLoaded:!1},sugar:{script:"lazy-sugar",isLoaded:!1},swfobject:{script:"lazy-swfobject",isLoaded:!1},popover:{script:"lazy-jquery.open-ux.popovers",styles:"lazy-jquery-open-ux-popovers",defaultElement:".ux-popover",defaultFunction:function(){jQuery(this.defaultElement).popover()},isLoaded:!1},textExpander:{script:"lazy-jquery.expander",styles:"lazy-jquery-expander",isLoaded:!1},joyRide:{script:"lazy-jquery.joyride",styles:"lazy-jquery-joyride",isLoaded:!1},validation:{script:"lazy-jquery.validationEngine",styles:["lazy-jquery-validationEngine"],dependencies:"validationRegEx",isLoaded:!1},validationRegEx:{script:"lazy-jquery.validationEngine-en",defaultSettings:{},defaultFunction:function(){jQuery.validationEngineLanguage.allRules=jQuery.extend(jQuery.validationEngineLanguage.allRules,this.defaultSettings)},isLoaded:!1},selectBoxEnhancement:{script:"lazy-jquery.chosen",styles:["lazy-jquery-chosen"],defaultElement:".ux-enhance-select",defaultSettings:{},defaultFunction:function(){jQuery(this.defaultElement).chosen()},isLoaded:!1},appMenu:{script:"lazy-jquery.open-ux.app-menus",defaultElement:"#app-nav a",defaultFunction:function(){jQuery(this.defaultElement).appMenu()},isLoaded:!1},modalLoader:{script:"lazy-jquery.open-ux.modal-loader",defaultElement:".processing",defaultFunction:function(){jQuery(this.defaultElement).on("click",function(){jQuery.modalLoader()})},isLoaded:!1},richTextEditor:{script:"ckeditor",styles:"lazy-ckeditor-override",specialPath:"ckeditor",isLoaded:!1},cookie:{script:"lazy-jquery.cookie",isLoaded:!1},json2:{script:"lazy-json2",isLoaded:!1},pageFeedback:{script:"lazy-jquery-page-feedback",styles:"lazy-jquery-page-feedback",dependencies:["cookie","json2"],isLoaded:!1},tooltips:{script:"lazy-jquery.open-ux.tooltips",styles:"lazy-jquery-open-ux-tooltips",defaultElement:"abbr, acronym, dfn, .ux-contexthelp, .tooltip",defaultFunction:function(){jQuery(this.defaultElement).tooltip()},isLoaded:!1},timePicker:{script:"lazy-jquery-ui-addon-time-picker",styles:"lazy-jquery-ui-addon-time-picker",defaultElement:".jQDateTimePicker",defaultSettings:{showAnim:"",showOn:"both",buttonText:"Datepicker",buttonImage:ux.core.resourcePath+"/images/icons/calendar_disabled.png",buttonImageOnly:!0,changeMonth:!0,changeYear:!0,controlType:"select",timeFormat:"hh:mm tt"},defaultFunction:function(){jQuery(this.defaultElement).datetimepicker(this.defaultSettings)},dependencies:["jqueryUI","jqueryDatePicker"],isLoaded:!1}},ux.functions={expandables:{defaultFunction:function(){ux.features.domSaver("a.ux-expandable","pageExpandables"),ux.dom.pageExpandables.each(function(){var obj=jQuery(this);obj.parent().is("li")?obj.parent().addClass("expandable-list"):obj.parent().hasClass("ux-closed")||"DIV"!=obj.parent()[0].nodeName||obj.addClass("expanded")}),$("body").on("click","a.ux-expandable",function(e){function checkSibs(obj){openClose(obj.parent("li").siblings("li").children(".ux-expandable.expanded")),openClose(obj)}function openClose(obj){var container=obj.next(),secondClass="expanded";obj.hasClass(secondClass)?container.slideUp(function(){obj.removeClass(secondClass).attr("aria-expanded",!1),container.attr("aria-hidden",!0)}):(container.slideDown().attr("aria-hidden",!1),obj.addClass(secondClass).attr("aria-expanded",!0))}e.preventDefault(),e.stopPropagation();var obj=jQuery(this),onlyOne=!1;obj.parents().is(".ux-only-one")&&(onlyOne=!0),onlyOne?checkSibs(obj):openClose(obj)})},isLoaded:!1},placeHolder:{defaultFunction:function(){jQuery('[class^="ux-attribute"]').each(function(){shim=jQuery(this).attr("class").replace(/ /g,":").replace(/_/g," ").split(":"),jQuery(this).attr(shim[1],shim[2])}),Modernizr.input.placeholder||(jQuery("[placeholder]").focus(function(){var input=jQuery(this);input.val()===input.attr("placeholder")&&(input.val(""),input.removeClass("ux-placeholder"),document.all&&input.get(0).createTextRange().select())}).blur(function(){var input=jQuery(this);""===input.val()?(input.addClass("ux-placeholder"),input.val(input.attr("placeholder"))):input.val()===input.attr("placeholder")?input.addClass("ux-placeholder"):input.removeClass("ux-placeholder")}).change(function(){var input=jQuery(this);input.hasClass("ux-placeholder")&&input.removeClass("ux-placeholder"),input.focus()}).blur(),jQuery("[placeholder]").parents("form").submit(function(){jQuery(this).find("[placeholder]").each(function(){var input=jQuery(this);input.val()===input.attr("placeholder")&&input.val("")})}))},isLoaded:!1},translate:{dependencies:"notifications",defaultFunction:function(){jQuery(".ux-translate-trigger").on("click",function(){$(".app-translate").length&&jQuery(this).uxNotice({title:"Select Language",message:jQuery("#ux-translate-container").html(),type:"Required",classes:"app-translate"})})},isLoaded:!1},dismissibleMessages:{defaultFunction:function(){jQuery(".ux-msg.ux-closable").each(function(){var msg=jQuery(this),closeElm=jQuery("<a>").addClass("ux-msg-close").html("&times;").on("click",function(){return msg.slideUp(),!1});msg.prepend(closeElm)})},isLoaded:!1},tableExpandables:{defaultFunction:function(){jQuery("tr.expandable ~ tr.expandable-child").hide(),jQuery("tr.expandable .icon").prepend('<a class="table-expandable-icon contracted" href="#"><img src="'+ux.core.resourcePath+'/images/background-navigation-expandable-right.gif" alt="show rows"></a>'),jQuery("tr.expandable .icon .contracted").live("click",function(){return jQuery(this).removeClass("contracted").addClass("expanded").children().attr("src",ux.core.resourcePath+"/images/background-navigation-expandable-down.gif").attr("alt","hide rows"),jQuery(this).parent().parent().nextAll(".expandable-child").show(),!1}),jQuery("tr.expandable .icon .expanded").live("click",function(){return jQuery(this).removeClass("expanded").addClass("contracted").children().attr("src",ux.core.resourcePath+"/images/background-navigation-expandable-right.gif").attr("alt","show rows"),jQuery(this).parent().parent().nextAll(".expandable-child").hide(),!1}),jQuery("input:disabled").addClass("disabled"),jQuery(".disabled-link").on("click",function(){return!1})},isLoaded:!1}}}
$(document).ready(function(){function menuIsActive(child){for(var node=child.parentNode;null!==node;){if($(node).hasClass(MENU_ACTIVE))return!0;node=node.parentNode}return!1}function toggleMainMenu(button){menuList.hasClass(MENU_ACTIVE)?(menuList.removeClass(MENU_ACTIVE),menuToggle.removeClass(MENU_ACTIVE)):button&&(menuList.addClass(MENU_ACTIVE),menuToggle.addClass(MENU_ACTIVE)),menuItems.removeClass(MENU_ACTIVE).removeClass(MENU_INACTIVE)}var menuToggle=$("#nys-menu-control"),menuList=$("#nys-global-nav"),menuItems=menuList.children("li"),MENU_ACTIVE="nys-global-active",MENU_INACTIVE="nys-global-inactive";menuItems.length||menuToggle.remove(),menuList.length&&(menuItems.children("a").on("click",function(e){if($(this).siblings("ul").length){var item=$(this).parent();return item.hasClass(MENU_ACTIVE)?menuItems.removeClass(MENU_ACTIVE).removeClass(MENU_INACTIVE):(menuItems.removeClass(MENU_ACTIVE),menuItems.addClass(MENU_INACTIVE),item.removeClass(MENU_INACTIVE),item.addClass(MENU_ACTIVE)),e.preventDefault(),e.stopPropagation(),!1}}),menuToggle.on("click",function(e){var t=1;return toggleMainMenu(t),e.preventDefault(),e.stopPropagation(),!1}),$(document).on("click",function(e){menuIsActive(e.target)||toggleMainMenu()}))});
!function($){ux.features.startup()}(jQuery);